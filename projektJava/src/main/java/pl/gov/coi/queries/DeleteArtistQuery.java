...
public class DeleteArtistQuery {
    ...
    private List<Object> getRandomParameters() throws SQLException {
        ...
        Integer num=rowsNumber();
        int i=0,w;
        int s;
        if(num!=1)
            s=Random.getIntValue(list.size()-1);
        else
            s=Random.getIntValue(list.size());
        w = (Integer)list.get(s);
        do
        {
            i=Random.getIntValue(num+1);
        }while(i!=w);       
        params.add(i);
        return params;
    }
    
    private Integer rowsNumber() throws SQLException
    {
